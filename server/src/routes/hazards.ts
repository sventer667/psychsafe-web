...
// (past reassessment cycles, not the one currently open). Lets the Hazard
// Register show "last rated Moderate on 12 Mar 2026" when someone re-adds a
// hazard the org has already assessed before, instead of starting blank
// every cycle. No new table needed: hazards already carry hazardLibraryId
// and belong to a case belonging to an org, so this is a plain join. The
// "most recent per hazardLibraryId" reduction is done in JS rather than SQL
// to avoid a fragile correlated-subquery tie-break.
hazardsRouter.get('/history', (req: AuthedRequest, res) => {
  const caseId = Number(req.query.caseId)
  if (!caseId || !ownsCase(req.auth!.orgId, caseId)) {
    return res.status(403).json({ error: 'Not authorized for this assessment' })
  }
  const rows = db
    .prepare(
      `SELECT h.hazardLibraryId, h.riskRating, h.residualRiskRating, h.createdAt as ratedAt, c.name as caseName
       FROM hazards h
       JOIN cases c ON c.id = h.caseId
       WHERE c.orgId = ? AND h.caseId != ? AND h.hazardLibraryId IS NOT NULL
       ORDER BY h.createdAt DESC`
    )
    .all(req.auth!.orgId, caseId) as {
  ...
  const latestByLibraryId = new Map<number, (typeof rows)[number]>()
  for (const row of rows) {
    if (!latestByLibraryId.has(row.hazardLibraryId)) {
      latestByLibraryId.set(row.hazardLibraryId, row)
    }
  }
  res.json([...latestByLibraryId.values()])
})
